...
public class ParametricasUsuarioDaoImpl implements ParametricasUsuarioDao {
	static final AfirmeNetLog LOG = new 
			AfirmeNetLog(ParametricasUsuarioDaoImpl.class);
	...
	public void insert(Comprobante comprobante) throws SQLException {
		...
		// mexicanada para no mover mas codigo - domingo electronico se debe guardar fecha cobro
		if(comprobante.getTransactionCode() ==  TipoTransferencia.DOMINGO_ELECTRONICO.getValor() ){
			sql.append(TimeUtils.getDateFormat( TimeUtils.getDate(comprobante.getValidationDate(), "dd/MM/yy") , "yyyy-MM-dd")); // 07/12/15 --> 2015-12-07
		}else{
			sql.append(TimeUtils.getDateFormat(new Date(), "yyyy-MM-dd"));
		}
		
		sql.append("', ").append(param.getDailytransnum()).append(", ");
		sql.append(comprobante.getAmount()).append(")");
	...
	public boolean delete(long contract, String bxiRef){
		...
		sb.append(" DC_ACCUM_ORIGIN_ACC_PERSON ");
		sb.append(" WHERE CONTRACT = ?");
		sb.append(" AND BXIREF = ?");

		db2Dao.getJdbcTemplate()
				.update(sb.toString(), new Object[] { contract,bxiRef });
