...
//								.getValue()));
	...
	public boolean isOnMonthlyLimit(TransferenciaBase tranferenciaBase,TipoTransferencia tipo)
		...
		if (property == null || property.equals("Y")) {
			...
			List<JOParamOwnAcc> parametros = parametricasTransaccionDao.getParameters(
							Long.valueOf(tranferenciaBase.getContractId()),
							Long.valueOf(debitAccount),
							tipo.getValor(), tranferenciaBase.getServiceType(), tranferenciaBase.getInterType());
			// Obtine el acumulado mensual
			List<JOParamAccumAcc> acumulados = parametricasTransaccionDao.getAcumulado(
							param.getContract(), param.getAccount(),
							param.getTranstype(), param.getSertyp(),
...
//								.getValue()));
			...
			double remainingAmount = po.getMonthlyamount() - montlyAmount; // ParamAccumAccBL.java:269 

			// if (po.getDailyamount() != 0.0) {
			if (po.getMonthlyamount() != 0.0) {

				// if (montlyAmount >= po.getDailyamount()) {
				if (montlyAmount >= po.getMonthlyamount() ) {
					goHead = false;
				}
				if (param.getDailyamount() > remainingAmount) {
					goHead = false;
	...
	@Override
	public boolean isValidDateSinHoliday(TransferenciaBase tranferenciaBase)
			throws AfirmeNetException {
		// This Validates that the Date is not over today's date
		if (TimeUtils.compareYYMMDD(tranferenciaBase.getValidationYear(),
				tranferenciaBase.getValidationDay(),
	...
	@SuppressWarnings("unchecked")
	@Override
	public boolean isHolyDateOrWeekEnd(String YY, String MM, String DD) {
	...
	public void applyValidations(List<TransferenciaBase> tranferencias, TipoTransferencia tipo, List<TipoValidacion> validaciones) {
		...
		for (TransferenciaBase tx : tranferencias) {
			...
			if(tx.getTipoTransferencia() == null){
				//Si el tipo de transferencia del service es spei preguntar si es TEF o SPEI de acuerdo al transactionCode
				if( tipo.equals(TipoTransferencia.TRANSFERENCIA_SPEI )){
					if( "05".equals(tx.getTransactionCode()) )
						tx.setTipoTransferencia(TipoTransferencia.TRANSFERENCIA_SPEI);
					if( "09".equals(tx.getTransactionCode()) )
						tx.setTipoTransferencia( TipoTransferencia.PAGO_INTERBANCARIO );
				}else{
			...
			for(TipoValidacion validacion: validaciones){
				...
				case FECHA:
					...
					break;
				case FECHA_SIN_HOLIDAY:
					try {
						if (!this.isValidDateSinHoliday(tx)) {
							errors.put(CodigoExcepcion.ERR_2931, AfirmeNetConstants.errores
									.get(CodigoExcepcion.ERR_2931.getValue()));
						}
					} catch (AfirmeNetException e) {
	...
	public boolean isValidTime(TransferenciaBase tranferenciaBase, TipoTransferencia tipo)
        ...
        case TRASPASO_PROPIAS: // Internal Transfer
        	addTime = 5;
            break;
         case TRASPASO_TERCEROS: // Terceros
        	 addTime = 5;
            break;
         case TRANSFERENCIAS_INTERNACIONALES_DOLARES: // External Transfer
        	 addTime = 5;
            break;
         case TRANSFERENCIAS_INTERNACIONALES_MULTIMONEDA: // External Transfer
        	 addTime = 5;
            break;
         case TRANSFERENCIA_SPEI: // Speua
        	 addTime = 5;
            break;
         case DOMICILIACION: // Domi
        	 addTime = 20;
            break;
         case NOMINA: // Nomina
        	 addTime = 20;
            break;
         case PAGO_INTERBANCARIO: // Tefo
        	 addTime = 20;
            break;
         case PAGO_DE_IMPUESTOS: // Impuestos
        	 addTime = 0;
            break;
         case PAGO_DE_SERVICIOS: // Servicios
        	 addTime = 5;
            break;
//         case 34: // Disp
//        	 addTime = 5;
...
//        	 addTime = 5;
//            break;
         case INVERSION_DIARIA: // Inv Diaria
        	 addTime = 0;
            break;
         case PAGO_DE_IMPUESTOS_DEPOSITO_REFERENCIADO: // Impuestos Deposito Referenciado
        	 addTime = 0;
             break;
         case TRASPASO_PUNTOS_BONUS: // Bonus
        	 addTime = 0;
             break;
         case VENTA_DE_SEGUROS_ANTIFRAUDE: // Cardif
        	 addTime = 0;
             break;
         case PAGO_REFERENCIADO: // Cardif
        	 addTime = 0;
             break;
         case MIS_CREDITOS_AFIRME: // MIS_CREDITOS_AFIRME
        	 addTime = 0;
             break;
         default:
        	 addTime = 100;
