...
public class ValidacionPagoServicioServiceImpl implements
	...
	public void esServicioValido(Servicio servicio) throws AfirmeNetException {
		...
		if(servicio.getTipoServicio()==null){
			//Revisamos si son servicio referenciado o colegios
			if(servicio.getInterType()==ConfigConvenios.PAGO_SERVICIO_COLEGIOS.getValor() || servicio.getInterType()==ConfigConvenios.PAGO_SERVICIO_REFERENCIADO.getValor()){
				errores=validacionesServicios.validaServicioReferenciado(servicio);
			}
		}else			
