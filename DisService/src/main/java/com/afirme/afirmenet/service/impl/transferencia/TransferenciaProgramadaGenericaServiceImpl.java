...
public class TransferenciaProgramadaGenericaServiceImpl implements
	...
	static final AfirmeNetLog LOG = new AfirmeNetLog(TransferenciaProgramadaGenericaServiceImpl.class);
		...
		try {
			...
			for (TransferenciaBase tx : transferencias) {
				if(tx.getErrores() == null || tx.getErrores().isEmpty()){
					...
					LOG.setSendInfo("cargo", tx.getTransactionCode(), tx.getAmount().toString(), tx.getContractId(), tx.getContractId(), tx.getDebitAccount(), tx.getCreditAccount(), tx.getAfirmeNetReference(), afitrans.getFormatName());
					afitrans.send();
					afitrans.destroy();				
				}			
			}
