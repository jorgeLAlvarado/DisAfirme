...
import com.afirme.afirmenet.service.impl.pagos.impuestos.PagoImpuestosServiceImpl;
...
public class SeguroCardifServiceImpl  implements TransferenciaAfirmeNet{
	...
	static final AfirmeNetLog LOG = new AfirmeNetLog(SeguroCardifServiceImpl.class);
	...
	public Comprobante sendBeanToAS400(SeguroCardif oBeanCardif, String ope) throws AfirmeNetException{
		...
		try {
			...
			afitrans.send();
			LOG.setSendInfo("cargo", oBeanCardif.getTransactionCode(), oBeanCardif.getAmount().toString(), oBeanCardif.getContractId(), oBeanCardif.getContractId(), oBeanCardif.getDebitAccount(), oBeanCardif.getCreditAccount(), oBeanCardif.getAfirmeNetReference(), afitrans.getFormatName());
			afitrans.destroy();
		} catch (Exception e) {
		...
		try {
        	...
        	LOG.setGetInfo("cargo", oBeanCardif.getTransactionCode(), oBeanCardif.getAmount().toString(), oBeanCardif.getContractId(), oBeanCardif.getContractId(), oBeanCardif.getDebitAccount(), oBeanCardif.getCreditAccount(), oBeanCardif.getAfirmeNetReference(), messageRecord);
        	...
        	if(!goHead){//Ocurrio un error
				...
				if(OPE_APLICACION.equalsIgnoreCase(ope)){
					comprobante.setBeneficiaryName(oBeanCardif.getPoliza());
					comprobante.setDescription(oBeanCardif.getPaquete());
					comprobante.setNarrative(oBeanCardif.getPaqueteDescripcion());
				}
			}
	...
	private SeguroCardif llenarBeanCardifDeAS400(SeguroCardif oBeanCardif, INDC30601Message afitrans, String ope) {
		...
		if(OPE_APLICACION.equalsIgnoreCase(ope)){
			oBeanCardif.setAfirmeNetReference(Util.getRefNum());
