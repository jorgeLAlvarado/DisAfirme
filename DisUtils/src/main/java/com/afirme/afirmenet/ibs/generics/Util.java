...
import com.afirme.afirmenet.utils.context.AppContext;
...
public class Util {
   ...
   */
	...
	/**
	 * Solamente valida [^A-Za-z ] letras y espacios.<br>
	 * <br>
	 * Utilizado al validar el nombre de ciudad destino en aviso de viaje.<br>
	 * <br>
	 * A diferencia del metodo soloLetrasEspacio() que soporta '.'<br>
	 * <br>
	 * @param cadena de tipo String
	 * @return boolean
	 */
	public static boolean soloLetrasEspacios(String cadena) {
	...
	/**
	 * Valida [^0.9 ()-+] la cadena puede contener numeros, espacios, (,), guiones,+<br>
	 * <br>
	 * Utilizado para valida el campo telefono contacto en aviso de viaje.<br>
	 * <br>
	 * @param cadena de tipo String
	 * @return boolean
	 */
	public static boolean validaTelefonoEnAvisoDeViaje(String cadena) {

		boolean resultado = true;
//		resultado = Pattern.matches("^[\\d\\(\\)\\-\\+]*$", cadena); // falto hacer pruebas por eso no se implemento
...
/**
	 * Para validar si la nueva contraseña tiene caracteres inválidos.
	 * 
	 ...
	 * @return boolean
	 */
	public static boolean validaCaracteresEnNuevaContrasena(String newPass){
		
		boolean resultado = false;
		String pattern = "^[A-Za-z\\dñÑ!@#$%^&*]*$";
		
		resultado = newPass.matches(pattern);
		
		return resultado;
	}
	
	/**
	 * Valida de lado del servidor la cadena del nuevo correo.
	 * 
	 * Utilizado en la actualizacion de correo del modulo configuracion-seguridad. 
	 * 
	 * @param newEmail
	 * @return
	 */
	public static boolean validaCaracteresEnNuevoCorreo(String newEmail){
		
		return Pattern.matches("^[\\w\\-\\.]+@[\\w\\.]+\\.\\w+$", newEmail);
	}
	...
	/* FORMATEO DE CANTIDADES CON "N" DECIMALES */
	public static String formatCCY(BigDecimal num, int N) {
		try {
			num.toString();
		} catch (Exception e) {
			return "0.00";
		}
		return formatCCY(num.toString(), N);
	}
	...
	public static String getCurrentDDMMYYYYFormatted() {
		String currentDate = getCurrentDDMMYYYY();
	...
	/**
	 * @Método que formatea una fecha de entrada: YYYYMMDD
	 * a la salida: dia "de" mes "de" Año 
	 * */
	
	public static String fechaLarga(String fecha){
		StringBuffer nuevaFecha= new StringBuffer();
		if(fecha.length()==8){
		int mes=Integer.parseInt(fecha.substring(fecha.length()-4,fecha.length()-2));
		
		nuevaFecha.append(fecha.substring(fecha.length()-2,fecha.length()))
		.append(" de ")
		.append(Util.getNombreMes(mes))
		.append(" de ")
	...
	public static String addMoneyMask(double d) {
	...
	public final static String getTimeStampAAMMDD() {
	...
	public final static String getTimeStampHHMMSS() {
	...
	public final static String getTimeStampAAAAMMDD() {
	...
	public final static String getTimeStampHHMMSS2() {
	...
	/**
	 * Metodo que permite la obtencion de los mensajes del archido de recursos<br>
	 * en el idioma que corresponda, asi como el paso de valores cuando existan<br>
	 * place holders en el mensaje.<br><br>
	 * 
	 * <b>Uso:</b><br>
	 * 
	 * el property debe ser de la forma:<br>
	 * 
	 * afirmenet.etiqueta.conf.correo.comprobante.nota.cambio.realizara.en.24.horas     =NOTA: Como medida de seguridad el cambio se realizará después de {0}.<br>
	 * 
	 * Util.getMessage("afirmenet.etiqueta.conf.correo.comprobante.nota.cambio.realizara.en.24.horas", new Object[]{"24 horas"})<br><br>
	 * 
	 * resultado:<br>
	 * NOTA: Como medida de seguridad el cambio se realizará después de 24 horas.<br><br>
	 * 
	 * <b>Code 809</b> indica que el el valor property no se pudo obtener, puede ser que no se encuntre el property.<br><br>
	 * 
	 * @param property
	 * @param values
	 * @return String valor del property en el archivo messages_ .properties dependiendo del idioma.
	 */
	public static String getPropertyString(String property, Object[] values){
		String resultado = "";
		...
		try{
			...
			resultado = messageSource.getMessage(property .trim(), values, locale);
		}catch( Exception e){
			resultado = "Code 809";
			e.printStackTrace();
		}
	...
	/**
	 * Sobrecarga del metodo getPropertyString(String property, Object[] values)<br><br>
	 * 
	 * Solo necesita el valor del property para obtener su valor.<br><br>
	 * 
	 * Si el property no es encontrado devuelve "Code 809" <br><br>
	 * 
	 * <b>Code 809</b> indica que el el valor property no se pudo obtener, puede ser que no se encuntre el property.<br><br>
	 * 
	 * <b>Uso:</b><br>
	 * 
	 * Util.obtenerValorProperty("afirmenet.etiqueta.conf.correo.comprobante.hora.de.activacion.del.cambio");<br><br>
	 * 
	 * @param property
	 * @return String valor del property en el archivo messages_ .properties dependiendo del idioma.
	 */
	public static String getPropertyString(String property){
		return Util.getPropertyString(property, new Object[]{});
	}
	
	 public static BigDecimal sumarMinutosFecha(String fechaAct, int minutos){
 		SimpleDateFormat sdf=new SimpleDateFormat("yyyyMMddHHmmss");
 		//System.out.println("Fecha envida antes de suma: "+ fechaAct);
 		Calendar calendar = Calendar.getInstance();
 		int year=Integer.parseInt(fechaAct.substring(0,4));
 		int month=Integer.parseInt(fechaAct.substring(4,6));
 		int date=Integer.parseInt(fechaAct.substring(6,8));
 		int hourOfDay=Integer.parseInt(fechaAct.substring(8,10));
 		int minute=Integer.parseInt(fechaAct.substring(10,12));
 		int second=Integer.parseInt(fechaAct.substring(12,14));
 		calendar.set(year, month-1, date, hourOfDay, minute, second);
 		String fecha=sdf.format(calendar.getTime());
 		//System.out.println("Fecha resultante parseo: "+ fecha);
 		calendar.add(Calendar.MINUTE, minutos);  // numero de horas a añadir, o restar en caso de horas<0
 		fecha=sdf.format(calendar.getTime());
 		//System.out.println("Fecha resultante suma: "+ fecha);
 		return new BigDecimal(fecha); // Devuelve el objeto Date con las nuevas horas añadidas
 	}

	/**
	 * Para obtener un Calendar con horario mexico.<br>
	 * <br>
	 * <b>Uso:</b>
	 * Calendar test = Util.convertCalendar(Calendar.getInstance(), TimeZone.getTimeZone("Mexico/General")); // Convierte la fecha a la actual en Mexico <br>
	 * Timestamp today = new Timestamp(test.getTimeInMillis());<br>
	 *  <br>
	 * @param calendar
	 * @param timeZone
	 * @return
	 */
	 ...
	 /**
	  * Para generar una cadena a partir de un array de cadenas.
	  * 
	  * @param a
	  * @param separator
	  * @return String
	  ...
	  * @return <code>true</code> if the String is null, empty
	  * @since 2.0
	  */
