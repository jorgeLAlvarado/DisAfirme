...
public class TransferenciaBase extends ComprobanteBase {
	...
	private String transaccionDescrip;
	...
	protected final String NUMBER_FORMAT = "###,###,##0.00";
	
	private String accion; //accion a realizar en 400
	
	private Map<String,Object> atributosGenericos = new HashMap<String, Object>();
	
	public String getAmountFormatted() {
		DecimalFormat decimalFormat = new DecimalFormat(NUMBER_FORMAT);
		return decimalFormat.format(getAmount().doubleValue());
	}
	
	public String getTaxCommisionFormatted() {
		DecimalFormat decimalFormat = new DecimalFormat(NUMBER_FORMAT);
		return decimalFormat.format(getTaxCommision().doubleValue());
	}
	
	public String getCommisionFormatted() {
		DecimalFormat decimalFormat = new DecimalFormat(NUMBER_FORMAT);
		return decimalFormat.format(getCommision().doubleValue());
	}
	
	public String getIvaFormatted() {
		DecimalFormat decimalFormat = new DecimalFormat(NUMBER_FORMAT);
		return decimalFormat.format(getIva().doubleValue());
	}
	...
	/**
	 * @param clientId el clientId a establecer
	 */
	...
	public String getProgrammingDateFormatted() {
		return Util.getFechaFormat(programmingDate);
	}
	public String getProgrammingHourFormatted() {
		return Util.getHoraFormat(programmingHour);
	}

	public void setValidationDateFull(String date){
		String fFecha="";
		try {
			fFecha += date.substring(6, 8);
			this.setValidationDay(date.substring(6, 8));
			fFecha += "/";
			fFecha += date.substring(4, 6);
			this.setValidationMonth(date.substring(4, 6));
			fFecha += "/";
			fFecha += date.substring(0, 4);
			this.setValidationYear(date.substring(0, 4));
		} catch (Exception ex) {
		}
		this.setValidationDate(fFecha);
	}
	
	public void setValidationHourFull(String hour){
		try {
			if (hour.trim().length() < 2)
				return;
			if (hour.length() <= 5)
				hour = "0" + hour;
			this.setValidationHour(hour.substring(0, 2));
			this.setValidationMinute(hour.substring(2, 4));
		} catch (Exception ex) {}
	}
	
	/**
	 * @return el transaccionDescrip
	 */
	public String getTransaccionDescrip() {
		return transaccionDescrip;
	}

	/**
	 * @param transaccionDescrip el transaccionDescrip a establecer
	 */
	public void setTransaccionDescrip(String transaccionDescrip) {
		this.transaccionDescrip = transaccionDescrip;
	}

	/**
	 * Para recuperar objetos seteados.
	 * 
	 * Ejemplo:
	 ...
	 * @return
	 */
	public Map<String, Object> getAtributosGenericos() {
		return atributosGenericos;
	}

	/**
	 * Necesario en la consulta de resultados de historial de comprobantes, para evitar otra consulta
	 * y completar datos.
	 * 
	 * Para transportar atributos de la forma:
	 * 
	 * transferenciaBase.getAtributosGenericos().put("aliasNuevo", DaoUtil.getString(map.get("NEWALIAS")) );
	 * 
	 * @param atributosGenericos
	 */
	public void setAtributosGenericos(Map<String, Object> atributosGenericos) {
		this.atributosGenericos = atributosGenericos;
	}
