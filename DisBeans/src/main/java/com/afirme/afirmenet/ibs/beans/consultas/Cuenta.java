 ...
 * 
 */
public class Cuenta extends TransferenciaBase implements java.io.Serializable {
	...
	public static final String TIPO_CREDITO = "Cr&eacute;ditos";
	public static final String TIPO_INVERSION = "Inversiones";
...
//		if (!StringUtils.isEmpty(this.number) && (this.number.length() > 4)) {
...
//		return "************";
	...
	public String getNumberMaskMail() {
		if (!StringUtils.isEmpty(this.number) && (this.number.length() > 4)) {
			return (this.number.substring(0, this.number.length() - 4))
					.replaceAll(".", "*")
					.concat(this.number.substring(this.number.length() - 4));
	 ...
	 * @return el dEUAMT
	 */
	public BigDecimal getdEUAMT() {
		return dEUAMT;
	}
	/**
	 * @param dEUAMT el dEUAMT a establecer
	 */
	public void setdEUAMT(BigDecimal dEUAMT) {
		this.dEUAMT = dEUAMT;
	}
	/**
	 * @return el tITCRD
